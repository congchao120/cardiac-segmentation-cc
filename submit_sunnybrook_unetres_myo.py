...
from metrics_common import dice_coef_endo_each, dice_coef_myo_each
...
def create_submission(contours, data_path, output_path, contour_type = 'm'):
    if contour_type == 'm':
        weights = 'model_logs/sunnybrook_m_unetres.h5'
    else:
    ...
    resArr = np.transpose([caseArr, imgArr, evalArr, evalMyoArr])
...
if __name__== '__main__':
    ...
    online_ctrs = list(map_all_contours(ONLINE_CONTOUR_PATH, contour_type))
    ...
    val_ctrs = list(map_all_contours(VAL_CONTOUR_PATH, contour_type))
