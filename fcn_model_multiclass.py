...
def dice_coef(y_true, y_pred, smooth=0.0):
    ...
    return (dice_coef_endo(y_true, y_pred, smooth) + dice_coef_myo(y_true, y_pred, smooth) + dice_coef_rv(y_true, y_pred, smooth))/3.0
...
def dice_coef_loss_endo(y_true, y_pred):
    return 1.0 - dice_coef_endo(y_true, y_pred, smooth=10.0)
...
def dice_coef_loss_myo(y_true, y_pred):
    return 1.0 - dice_coef_myo(y_true, y_pred, smooth=10.0)
...
def fcn_model_multiclass(input_shape, num_classes, transfer=True, contour_type='i', weights=None):
...
if __name__ == '__main__':
    model = fcn_model_multiclass((100, 100, 1), 4, weights=None)
