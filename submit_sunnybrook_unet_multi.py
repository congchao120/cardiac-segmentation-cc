...
from train_sunnybrook_unet_mul import read_contour, map_all_contours, export_all_contours, map_endo_contours
from helpers import reshape, get_SAX_SERIES, draw_result
from unet_multi_model import unet_multi_model, dice_coef_endo_each, dice_coef_myo_each
...
def create_submission(contours, data_path, output_path, contour_type = 'a'):
    if contour_type == 'a':
        weights = 'model_logs/sunnybrook_a_unet_multi.h5'
    else:
    ...
    model = unet_multi_model(input_shape, num_classes, weights=weights, contour_type=contour_type, transfer=True)
