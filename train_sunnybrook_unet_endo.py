...
from unet_model_inv import unet_model_inv
seed = 1234
np.random.seed(seed)
...
if __name__== '__main__':
    ...
    train_ctrs = list(map_endo_contours(TRAIN_CONTOUR_PATH))
    if shuffle:
    ...
    model = unet_model_inv(input_shape, num_classes, num_filters=8, transfer=True, contour_type=contour_type, weights=weight_path)
    kwargs = dict(
        rotation_range=90,
        zoom_range=0.1,
    ...
    if K.backend() == 'tensorflow':
        tensorboard = TensorBoard(log_dir=os.path.join(save_path, 'logs_unet_inv'), histogram_freq=10, write_graph=False,
    ...
    save_file = '_'.join(['sunnybrook', contour_type, 'unet', 'inv']) + '.h5'
